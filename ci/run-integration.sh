...
#   VGI_MATCH_WORKER     worker LOCATION the .test files ATTACH (a stdio command
#                        such as `.venv/bin/vgi-match`, or an http:// URL)
# Optional:
#   STAGE                scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_MATCH_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_MATCH_WORKER) ..."
